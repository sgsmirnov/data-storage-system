...
import org.alfresco.repo.domain.dialect.*;
...
import org.alfresco.util.schemacomp.*;
...
public class SchemaBootstrap extends AbstractLifecycleBean {
     ...
     * @return Returns a string that is short enough for {@link org.alfresco.repo.domain.schema.SchemaBootstrap#getMaxStringLength()}
     * @since 3.2
     */
    ...
    private static Log logger = LogFactory.getLog(org.alfresco.repo.domain.schema.SchemaBootstrap.class);
     ...
     *
     * @param updateSchema true to update and validate the schema, otherwise false to just validate the schema. Default is <b>true</b>.
     */
     ...
     *
     * @param stopAfterSchemaBootstrap <tt>true</tt> to terminate (with exception) after running all the usual schema updates and checks.
     */
     ...
     *
     * @author Derek Hulley
     * @see org.alfresco.repo.domain.schema.SchemaBootstrap#setStopAfterSchemaBootstrap(boolean)
     * @since 3.1.1
     */
    ...
    private void checkDialect(Dialect dialect) {
        ...
        int maxStringLength = org.alfresco.repo.domain.schema.SchemaBootstrap.DEFAULT_MAX_STRING_LENGTH;
        ...
        if (dialect instanceof SQLServerDialect) {
            ...
            maxStringLength = org.alfresco.repo.domain.schema.SchemaBootstrap.DEFAULT_MAX_STRING_LENGTH;
        } else if (dialect instanceof MySQLClusterNDBDialect) {
            // string_value varchar(400),
            // serializable_value blob,
            maxStringLength = org.alfresco.repo.domain.schema.SchemaBootstrap.DEFAULT_MAX_STRING_LENGTH_NDB;
        ...
        org.alfresco.repo.domain.schema.SchemaBootstrap.setMaxStringLength(maxStringLength, dialect);
        ...
        if (maximumStringLength > 0) {
            org.alfresco.repo.domain.schema.SchemaBootstrap.setMaxStringLength(maximumStringLength, dialect);
        }
    }
     ...
     * @param outputFileNameTemplate - {@link String} value, which determines template of temporary filename for validation report. <b>It can't be <code>null</code> if <code>out</code> is <code>null</code></b>!
     * @param out                    - {@link PrintWriter} instance, which represents an external output stream for writing a validation report. This stream is never closed or flushed. <b>It can't be <code>null</code> if <code>outputFileNameTemplate</code> is <code>null</code></b>!
